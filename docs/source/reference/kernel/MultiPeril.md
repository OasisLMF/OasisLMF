...
There are a few ways in which losses can be combined and the first example in ktools uses a simple rule, which is to sum the losses for each coverage and cap the overall loss to the total insured value. This is what you get when you use the -c parameter in gulmc to output losses by 'coverage'.  

In v3.1.0 the method of combining losses became function-driven using the gulmc command line parameter -a as a few standard approaches have emerged. These are;
...
Note that ground up losses by item can be passed into the financial module unallocated (allocation method 0) using the gulmc option -i, or allocated using the gulmc option -a1 -i.  If the item ground up losses are passed though unallocated then the limit of total insured value must be applied as part of the financial module calculations, to prevent the ground up loss exceeding the coverage TIV.  
