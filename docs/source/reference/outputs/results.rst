...
* :ref:`ord_outputs_results` — the current output standard
...
* :ref:`legacy_output_formats` — pre-ORD formats, for older results

.. _ord_outputs_results:

ORD outputs
-----------

**ORD (Open Results Data) is the current output standard and what new work should use.**
Open Results Data is a data standard for catastrophe loss model results developed as part
of Open Data Standards "ODS", curated by OasisLMF and governed by the Open Data Standards
Steering Committee, comprised of industry experts representing (re)insurers, brokers,
service providers and catastrophe model vendors.

The **table definitions and field semantics** are single-sourced in the
`ODS_OpenResultsData <https://oasislmf.github.io/ord/reference/tables.html>`_ repository
and generated from its schema. This page documents what the **Oasis toolchain writes** for
each table — the concrete field names, types, byte widths and examples — and which
``analysis_settings`` flag requests it.

The **components** that emit these tables are documented in
:doc:`../kernel/OutputComponents`; the tables below do not repeat their command lines.

.. list-table:: ORD tables produced by Oasis
    :header-rows: 1
    :widths: 24 30 22
    ...
    * - ``elt_sample``
      ...
      - ``summarypy`` (summary stream), ``eltpy``
    * - ``elt_moment``
    ...
    * - ``plt_sample`` / ``plt_moment`` / ``plt_quantile``
      - SPLT / MPLT / QPLT — Period Loss Tables
      - ``pltpy``
    * - ``ept_*`` / ``psept_*``
      - EPT / PSEPT — Exceedance Probability Tables
      - ``lecpy``
    * - ``alt_period`` / ``alt_meanonly``
      - ALT / PALT — Average Loss Tables
      - ``aalpy``

The legacy, pre-ORD output formats are retained for reference in
:ref:`legacy_output_formats` below.
...
Moment and Quantile Event Loss Tables (MELT, QELT)
**************************************************
...
Period Loss Tables (SPLT, MPLT, QPLT)
*************************************
...
Exceedance Probability Tables (EPT, PSEPT)
******************************************
...
Legacy (pre-ORD) output formats
-------------------------------

.. note::
    These are the **pre-ORD** output formats, kept for reading older results and for
    models that have not moved over. For new work use the ORD tables above. The
    components that produce output are documented in :doc:`../kernel/OutputComponents`;
    the command lines shown historically referred to the retired ktools binaries and have
    been removed.
