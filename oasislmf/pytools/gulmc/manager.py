...
from oasislmf.pytools.getmodel.common import (
    Correlation, HazardCorrelation, Index_type, nb_areaperil_int, oasis_float, Keys)
...
def run(run_dir,
    ...
    with ExitStack() as stack:
        ...
        def gen_empty_vuln_cdf_lookup(list_size):
            ...
            # cached_vuln_cdf_pointers = List.empty_list(nb_int32)
            cached_vuln_cdf_lengths = List.empty_list(VULN_LOOKUP_VALUE_TYPE)
            dummy = tuple((nb_int32(0), nb_int32(0)))
            for i in range(list_size):
                cached_vuln_cdf_lengths.append(dummy)
            ...
            if event_footprint is not None:
                ...
                while last_processed_coverage_ids_idx < compute_i:
                    ...
                    cursor, cursor_bytes, last_processed_coverage_ids_idx, next_cached_vuln_cdf = compute_event_losses(
                        ...
                        loss_threshold, losses, vuln_prob_to, weighted_vuln_to_empty, alloc_rule, do_correlation, haz_rndms_base, vuln_rndms_base, eps_ij, haz_eps_ij,
                        corr_data_by_item_id, haz_corr_data_by_item_id, arr_min, arr_max, arr_N, norm_inv_cdf, arr_min_cdf, arr_max_cdf, arr_N_cdf, norm_cdf,
...
def compute_event_losses(event_id,
    ...
    for coverage_i in range(last_processed_coverage_ids_idx, coverage_ids.shape[0]):
        ...
        for item_i in range(Nitems):
            ...
            # eff_vuln_cdf_i and eff_vuln_cdf_Ndamage_bins are two integers that define 1 vulnerability cdf
            # therefore, for aggregate vuln, there are N pairs (eff_vuln_cdf_i, eff_vuln_cdf_Ndamage_bins)
            # if the aggregate vulnerability is made of N sub-vulnerability
            ...
            # eff_vuln_cdf_Ndamage_bins = areaperil_to_eff_vuln_cdf_Ndamage_bins[(areaperil_id, vulnerability_id)]
            ...
            if vulnerability_id in agg_vuln_to_vulns:
                ...
                eff_damag_vuln_cdf_i = areaperil_to_eff_vuln_cdf[(areaperil_id, vuln_i)]
                eff_damag_cdf_Ndamage_bins = areaperil_to_eff_vuln_cdf_Ndamage_bins[(areaperil_id, vuln_i)]
...
def get_vuln_cdf(vulnerability_id, haz_bin_idx, haz_int_bin_idx, vuln_cdf_lookup, vuln_array, vuln_cdf_empty,
    ...
    return vuln_prob_to, Ndamage_bins, last_cdf_entry
            ...
            if last_areaperil_id > 0:
                if last_areaperil_id in areaperil_to_vulns_idx_dict:
                    ...
                    for vuln_idx in range(areaperil_to_vulns_idx['start'], areaperil_to_vulns_idx['end']):
                        ...
                        # then, compute the cdf for the aggregate vulnerabilities in this areaperil_id
                        # TODO: need to build areaperil_to_agg_vuln: dict from areaperilid -> list of agg vuln_id
                        # BUT problem is we cannot store all the aggregate cdfs because they're too many...
                        # ...we cannot do this here...we need to compute this in compute_losses
                        # for agg_vuln_id in areaperil_to_agg_vuln[last_areaperil_id]:
                        #     ... loop over all the sub-vuln_id of this agg_vuln_id,
                        #     the cdf is the linear combination of the sub-vuln-id cdf and the weights
