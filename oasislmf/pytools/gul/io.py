...
def read_getmodel_stream(stream_in, item_map, coverages, compute, seeds, buff_size, valid_area_peril_id=None):
    ...
    if monte_carlo is True:
        sample_size = np.frombuffer(stream_in.read(4), dtype='i4')

        min_size_cdf_entry = 4 + 4 + sample_size * 4  # event_id, areaperil_id, haz intensity bin id samples...
    else:
    ...
    while True:
        ...
        if monte_carlo is True:
            cursor, yield_event, event_id, rec, rec_idx_ptr, last_event_id, compute_i, items_data_i, items_data, rng_index, group_id_rng_index, damagecdf_i = stream_to_data_monte_carlo(
...
@njit(cache=True, fastmath=True)
def stream_to_data_monte_carlo(int32_mv, valid_buf, size_cdf_entry, last_event_id, item_map, coverages, valid_area_peril_dict,
                               compute_i, compute, items_data_i, items_data, seeds, rng_index, group_id_rng_index, damagecdf_i, rec_idx_ptr, sample_size):
    """Parse streamed data into data arrays.
    ...
    while cursor * int32_mv.itemsize + size_cdf_entry <= valid_buf:
        ...
        # vulnerability_id, cursor = int32_mv[cursor], cursor + 1
        # Nbins_to_read, cursor = int32_mv[cursor], cursor + 1
        Nbins_to_read = sample_size

        if cursor * int32_mv.itemsize + Nbins_to_read * ProbMean_size > valid_buf:
            # if the next cdf record is not fully contained in the valid buf, then
            # get more data in the buffer and put cursor back at the beginning of this cdf
            cursor -= areaperil_int_to_int32_size + 1
            break
