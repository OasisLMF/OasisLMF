...
def cdf_tocsv(stack, file_in, file_out, file_type, noheader, run_dir):
    ...
    # This converter dumps the cdf stream and never samples, so every item is one building and
    # the per-seed counts go unread. The array is still indexed by item_id inside njit, which
