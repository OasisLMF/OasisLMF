...
from oasislmf.pytools.common.data import event_id as event_id_dtype, item_id as item_id_dtype
...
event_id_dtype = np.dtype(event_id_dtype[1])
item_id_dtype = np.dtype(item_id_dtype[1])
...
def read_buffer(byte_mv, cursor, valid_buff, event_id, item_id, data, idxs, state):
    ...
    while cursor < valid_buff:
        if not state["reading_losses"]:
            # Read summary header
            if valid_buff - cursor >= event_id_dtype.itemsize + item_id_dtype.itemsize:
                event_id_new, cursor = mv_read(byte_mv, cursor, event_id_dtype,
                                               event_id_dtype.itemsize)
                if last_event_id != 0 and event_id_new != last_event_id:
                    # New event, return to process the previous event
                    idx = idxs[0]
                    return cursor - event_id_dtype.itemsize, last_event_id, item_id, 1
                event_id = event_id_new
                item_id, cursor = mv_read(byte_mv, cursor, item_id_dtype,
                                          item_id_dtype.itemsize)
