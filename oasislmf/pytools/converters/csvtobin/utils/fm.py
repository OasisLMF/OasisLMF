...
def _count_fm_groups_and_valid(event_ids, item_ids, sidxs, max_sample_index):
...
@nb.njit(cache=True, error_model="numpy")
def _fill_fm_stream(event_ids, item_ids, sidxs, losses_i32, max_sample_index, out):
    """Fill the pre-allocated int32 output buffer.

    losses_i32 is the float32 loss column reinterpreted as int32 (bit-identical),
    allowing the buffer to stay int32 throughout while preserving the float bits.
...
def fm_tobin(stack, file_in, file_out, file_type, stream_type, max_sample_index):
    ...
    item_ids = np.ascontiguousarray(data["output_id"])
    sidxs = np.ascontiguousarray(data["sidx"])
    # astype(float32) copies to a contiguous array; view(int32) reinterprets the bits
    losses_i32 = data["loss"].astype(np.float32).view(np.int32)

    G, n_valid = _count_fm_groups_and_valid(event_ids, item_ids, sidxs, max_sample_index)
    if G == 0:
        return

    # Each group occupies 4 int32s (2 header + 2 terminator) plus 2 per valid row
    out = np.empty(G * 4 + n_valid * 2, dtype=np.int32)
    _fill_fm_stream(event_ids, item_ids, sidxs, losses_i32, max_sample_index, out)
    out.tofile(file_out)
