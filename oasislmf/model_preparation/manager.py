...
class OasisManagerInterface(Interface):  # pragma: no cover
    ...
    def get_canonical_exposure_profile(self, oasis_model=None, **kwargs):
        """
    ...
    def get_canonical_accounts_profile(self, oasis_model=None, **kwargs):
        """
        Loads a JSON string or JSON file representation of the canonical
    ...
    def generate_gul_input_items(self, canonical_exposure_profile, canonical_exposure_df, keys_df, **kwargs):
        """
        Generates GUL input items.

        :param canonical_exposure_profile: Canonical exposures profile
        :type canonical_exposure_profile: dict

        :param canonical_exposure_df: Canonical exposures
        :type canonical_exposure_df: pandas.DataFrame
    ...
    def generate_fm_input_items(self, canonical_exposure_df, gul_items_df, canonical_exposure_profile, canonical_accounts_profile, canonical_accounts_df, fm_agg_profile, **kwargs):
        """
    ...
    def get_gul_input_items(self, canonical_exposure_profile, canonical_exposure_file_path, keys_file_path, **kwargs):
        """
        ...
        :param canonical_exposure_file_path: Canonical exposures file path
        :type canonical_exposure_file_path: str
    ...
    def get_fm_input_items(self, canonical_exposure_df, gul_items_df, canonical_exposure_profile, canonical_accounts_profile, canonical_accounts_file_path, fm_agg_profile, **kwargs):
        """
...
class OasisManager(implements(OasisManagerInterface)):
    ...
    def transform_source_to_canonical(self, oasis_model=None, source_type='exposures', **kwargs):
        ...
        :param source_exposure_file_path: Source exposures file path (if ``source_type`` is ``exposures``)
        :type source_exposure_file_path: str

        :param source_exposure_validation_file_path: Source exposures validation file (if ``source_type`` is ``exposures``)
        :type source_exposure_validation_file_path: str

        :param source_to_canonical_exposure_transformation_file_path: Source exposures transformation file (if ``source_type`` is ``exposures``)
        :type source_to_canonical_exposure_transformation_file_path: str

        :param canonical_exposure_file_path: Path to the output canonical exposure file (if ``source_type`` is ``exposures``)
        :type canonical_exposure_file_path: str
        ...
        input_file_path = os.path.abspath(kwargs['source_accounts_file_path']) if source_type == 'accounts' else os.path.abspath(kwargs['source_exposure_file_path'])
        transformation_file_path = os.path.abspath(kwargs['source_to_canonical_accounts_transformation_file_path']) if source_type == 'accounts' else os.path.abspath(kwargs['source_to_canonical_exposure_transformation_file_path'])

        try:
            validation_file_path = kwargs['source_accounts_validation_file_path'] if source_type == 'accounts' else kwargs.get['source_exposure_validation_file_path']
        except (KeyError, TypeError):
            validation_file_path = None

        output_file_path = os.path.abspath(kwargs['canonical_accounts_file_path']) if source_type == 'accounts' else os.path.abspath(kwargs['canonical_exposure_file_path'])
        ...
        if oasis_model:
            if source_type == 'accounts':
                ...
                oasis_model.resources['oasis_files_pipeline'].canonical_exposure_file_path = output_file_path
    ...
    def transform_canonical_to_model(self, oasis_model=None, **kwargs):
        ...
        :param canonical_exposure_file_path: Path to the canonical exposures file
        :type canonical_exposure_file_path: str

        :param canonical_exposure_validation_file_path: Path to the exposure validation file
        :type canonical_exposure_validation_file_path: str

        :param canonical_to_model_exposure_transformation_file_path: Path to the exposure transformation file
        :type canonical_to_model_exposure_transformation_file_path: str

        :param model_exposure_file_path: Path to the output model exposure file
        :type model_exposure_file_path: str

        :return: The path to the output model exposure file
        """
        kwargs = self._process_default_kwargs(oasis_model=oasis_model, **kwargs)

        input_file_path = os.path.abspath(kwargs.get('canonical_exposure_file_path'))
        transformation_file_path = os.path.abspath(kwargs.get('canonical_to_model_exposure_transformation_file_path'))

        try:
            validation_file_path = kwargs['canonical_exposure_validation_file_path']
        except (KeyError, TypeError):
            validation_file_path = None

        output_file_path = os.path.abspath(kwargs.get('model_exposure_file_path'))
        ...
        if oasis_model:
            oasis_model.resources['oasis_files_pipeline'].model_exposure_file_path = output_file_path
    ...
    def get_canonical_exposure_profile(self, oasis_model=None, **kwargs):
        ...
        profile_json = kwargs.get('canonical_exposure_profile_json')
        profile_json_path = kwargs.get('canonical_exposure_profile_path')
        ...
        if oasis_model:
            oasis_model.resources['canonical_exposure_profile'] = profile
    ...
    def get_canonical_accounts_profile(self, oasis_model=None, **kwargs):
        ...
        profile_json_path = kwargs.get('canonical_accounts_profile_path')
    ...
    def get_keys(self, oasis_model=None, **kwargs):
        ...
        model_exposure_file_path = kwargs.get('model_exposure_file_path')
        lookup = kwargs.get('lookup')
    ...
    def _process_default_kwargs(self, oasis_model=None, **kwargs):
        if oasis_model:
            ...
            kwargs.setdefault('source_exposure_validation_file_path', omr.get('source_exposure_validation_file_path'))
            kwargs.setdefault('source_accounts_validation_file_path', omr.get('source_accounts_validation_file_path'))

            kwargs.setdefault('source_to_canonical_exposure_transformation_file_path', omr.get('source_to_canonical_exposure_transformation_file_path'))
            ...
            kwargs.setdefault('canonical_exposure_profile', omr.get('canonical_exposure_profile'))
            kwargs.setdefault('canonical_accounts_profile', omr.get('canonical_accounts_profile'))

            kwargs.setdefault('canonical_exposure_profile', omr.get('canonical_exposure_profile'))
            kwargs.setdefault('canonical_accounts_profile', omr.get('canonical_accounts_profile'))

            kwargs.setdefault('canonical_exposure_profile_json', omr.get('canonical_exposure_profile_json'))
            ...
            kwargs.setdefault('canonical_exposure_profile_path', omr.get('canonical_exposure_profile_path'))
            kwargs.setdefault('canonical_accounts_profile_path', omr.get('canonical_accounts_profile_path'))

            kwargs.setdefault('canonical_exposure_file_path', ofp.canonical_exposure_file_path)
            kwargs.setdefault('canonical_accounts_file_path', ofp.canonical_accounts_file_path)

            kwargs.setdefault('canonical_exposure_validation_file_path', omr.get('canonical_exposure_validation_file_path'))
            kwargs.setdefault('canonical_to_model_exposure_transformation_file_path', omr.get('canonical_to_model_exposure_transformation_file_path'))
            ...
            kwargs.setdefault('model_exposure_file_path', ofp.model_exposure_file_path)

            kwargs.setdefault('keys_file_path', ofp.keys_file_path)
            kwargs.setdefault('keys_errors_file_path', ofp.keys_errors_file_path)

            kwargs.setdefault('canonical_exposure_df', omr.get('canonical_exposure_df'))
    ...
    def generate_gul_input_items(
        ...
        :param canonical_exposure_df: Canonical exposures data frame
        :type canonical_exposure_df: pandas.DataFrame

        :param keys_df: Keys file data frame
        :type keys_df: pandas.DataFrame
        """
        canexp_df = canonical_exposure_df

        cep = canonical_exposure_profile
    ...
    def get_gul_input_items(self, canonical_exposure_profile, canonical_exposure_file_path, keys_file_path):
        """
        Loads GUL input items generated by ``generate_gul_input_items`` into a static
        structure such as a pandas dataframe.
        ...
        try:
            with io.open(canonical_exposure_file_path, 'r', encoding='utf-8') as cf, io.open(keys_file_path, 'r', encoding='utf-8') as kf:
            ...
            gul_items_df = pd.DataFrame(data=[it for it in self.generate_gul_input_items(cep, canexp_df, keys_df)], dtype=object)
    ...
    def get_fm_input_items(
        ...
        try:
            ...
            fm_items = [it for it in self.generate_fm_input_items(canexp_df, gul_items_df, cep, cap, canacc_df, fmap)]
    ...
    def write_gul_input_files(self, oasis_model=None, **kwargs):
        ...
        canonical_exposure_profile = kwargs.get('canonical_exposure_profile')
        canonical_exposure_file_path = kwargs.get('canonical_exposure_file_path')
        keys_file_path = kwargs.get('keys_file_path')

        gul_items_df, canexp_df = self.get_gul_input_items(canonical_exposure_profile, canonical_exposure_file_path, keys_file_path)

        if oasis_model:
            omr['canonical_exposure_df'] = canexp_df
    ...
    def write_fm_input_files(self, oasis_model=None, **kwargs):
        ...
        if oasis_model:
            ...
            canexp_df, gul_items_df = omr['canonical_exposure_df'], omr['gul_items_df']
            canonical_exposure_profile = omr['canonical_exposure_profile']
            ...
            canexp_df, gul_items_df = kwargs.get('canonical_exposure_df'), kwargs.get('gul_items_df')
            canonical_exposure_profile = kwargs.get('canonical_exposure_profile')
        ...
        fm_items_df, canacc_df = self.get_fm_input_items(canexp_df, gul_items_df, canonical_exposure_profile, canonical_accounts_profile, canonical_accounts_file_path, fm_agg_profile)
    ...
    def start_oasis_files_pipeline(
        ...
        **kwargs
        ...
        if not source_exposure_file_path:
            raise OasisException('No source exposure file path provided')
        elif not os.path.exists(source_exposure_file_path):
            raise OasisException("Source exposure file path {} does not exist on the filesysem".format(source_exposure_file_path))

        logger.info('\nFound source exposures file {source_exposure_file_path} - copying to Oasis files directory'.format(**kwargs))
        shutil.copy2(source_exposure_file_path, oasis_files_path)

        logger.info('\nLoading canonical exposures profile')
        canonical_exposure_profile = kwargs.get('canonical_exposure_profile') or self.get_canonical_exposure_profile(oasis_model=oasis_model, **kwargs)

        if canonical_exposure_profile is None:
            raise OasisException('No canonical exposures profile provided')

        logger.info('\nFound canonical exposures profile: {}'.format(canonical_exposure_profile))
        ...
        if fm:
            ...
            canonical_accounts_profile = kwargs.get('canonical_accounts_profile') or self.get_canonical_accounts_profile(oasis_model=oasis_model, **kwargs)
        ...
        canonical_exposure_file_path = os.path.join(oasis_files_path, 'canexp-{}.csv'.format(utcnow))
        canonical_accounts_file_path = os.path.join(oasis_files_path, 'canacc-{}.csv'.format(utcnow))

        model_exposure_file_path = os.path.join(oasis_files_path, 'modexp-{}.csv'.format(utcnow))
        ...
        if oasis_model:
            ofp.source_exposure_file_path = source_exposure_file_path
            ofp.source_accounts_file_path = source_accounts_file_path

            ofp.canonical_exposure_file_path = canonical_exposure_file_path
            ofp.canonical_accounts_file_path = canonical_accounts_file_path

            ofp.model_exposure_file_path = model_exposure_file_path
        ...
        logger.info('\nWriting canonical exposures file {canonical_exposure_file_path}'.format(**kwargs))
        ...
        logger.info('\nWriting model exposures file {model_exposure_file_path}'.format(**kwargs))
        ...
        logger.info('\nWriting GUL input files')
        gul_input_files = self.write_gul_input_files(oasis_model=oasis_model, **kwargs)

        if not fm:
            return gul_input_files

        logger.info('\nWriting FM input files')
        fm_input_files = self.write_fm_input_files(oasis_model=oasis_model, **kwargs)
    ...
    def create_model(self, model_supplier_id, model_id, model_version, resources=None):
        ...
        if omr.get('canonical_exposure_profile') is None:
            self.get_canonical_exposure_profile(oasis_model=model)

        if omr.get('canonical_accounts_profile') is None:
            self.get_canonical_accounts_profile(oasis_model=model)
