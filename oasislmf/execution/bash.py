...
def create_bash_analysis(
    ...
    **kwargs
    ...
    print_command(filename, '# --- Printing the dirs ---')
    print_command(filename, f'# process_counter {process_counter}')
    print_command(filename, f'# max_process_id {max_process_id}')
    print_command(filename, f'# num_reinsurance_iterations {num_reinsurance_iterations}')
    print_command(filename, f'# fifo_tmp_dir {fifo_tmp_dir}')
    print_command(filename, f'# gul_legacy_stream {gul_legacy_stream}')
    print_command(filename, f'# _get_getmodel_cmd {_get_getmodel_cmd}')
    print_command(filename, f'# custom_args {custom_args}')
    print_command(filename, f'# fmpy {fmpy}')
    print_command(filename, f'# fmpy_low_memory {fmpy_low_memory}')
    print_command(filename, f'# fmpy_sort_output {fmpy_sort_output}')
    print_command(filename, f'# process_number {process_number}')
    print_command(filename, f'# fifo_queue_dir {fifo_queue_dir}')
    print_command(filename, f'# fifo_full_correlation_dir {fifo_full_correlation_dir}')
    print_command(filename, f'# stderr_guard {stderr_guard}')
    print_command(filename, f'# gul_item_stream {gul_item_stream}')
    print_command(filename, f'# work_dir {work_dir}')
    print_command(filename, f'# work_kat_dir {work_kat_dir}')
    print_command(filename, f'# work_full_correlation_dir {work_full_correlation_dir}')
    print_command(filename, f'# work_full_correlation_kat_dir {work_full_correlation_kat_dir}')
    print_command(filename, f'# output_dir {output_dir}')
    print_command(filename, f'# output_full_correlation_dir {output_full_correlation_dir}')
    print_command(filename, f'# gul_alloc_rule {gul_alloc_rule}')
    print_command(filename, f'# il_alloc_rule {il_alloc_rule}')
    print_command(filename, f'# ri_alloc_rule {ri_alloc_rule}')
    print_command(filename, f'# num_gul_per_lb {num_gul_per_lb}')
    print_command(filename, f'# num_fm_per_lb {num_fm_per_lb}')
    print_command(filename, f'# eve_shuffle_flag {eve_shuffle_flag}')
    print_command(filename, f'# kat_sort_by_event {kat_sort_by_event}')
    print_command(filename, f'# gul_threshold {gul_threshold}')
    print_command(filename, f'# number_of_samples {number_of_samples}')
    print_command(filename, f'# use_random_number_file {use_random_number_file}')
    print_command(filename, f'# full_correlation {full_correlation}')
    print_command(filename, f'# gul_output {gul_output}')
    print_command(filename, f'# il_output {il_output}')
    print_command(filename, f'# ri_output {ri_output}')
    print_command(filename, f'# need_summary_fifo_for_gul {need_summary_fifo_for_gul}')
    print_command(filename, f'# analysis_settings {analysis_settings}')
    print_command(filename, f'# modelpy {modelpy}')
    print_command(filename, f'# model_py_server {model_py_server}')
