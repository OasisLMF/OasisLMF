...
def group_by_oed(oed_col_group, summary_map_df, exposure_df, sort_by, accounts_df=None):
    ...
    summary_tiv = summary_map_df.drop_duplicates(['locnumber', 'coverage_type_id'] + oed_col_group, keep='first').groupby(oed_col_group).sum('tiv').tiv
...
def get_summary_xref_df(map_df, exposure_df, accounts_df, summaries_info_dict, summaries_type, gul_items=False):
    ...
    for summary_set in summaries_info_dict:
        ...
        if not set(cols_group_by).intersection(all_cols):
            ...
            summary_desc[desc_key].insert(loc=len(summary_desc[desc_key].columns), column='tiv', value=map_df.drop_duplicates(['locnumber', 'coverage_type_id'], keep='first').tiv.sum())
