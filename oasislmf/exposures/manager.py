...
class OasisExposuresManagerInterface(Interface):  # pragma: no cover
    ...
    def load_fm_items(self, canonical_exposures, gul_items, canonical_exposures_profile, canonical_accounts_profile, canonical_accounts_file_path, **kwargs):
        """
        Loads FM items generated by ``generate_fm_items`` into a static
        structure such as a pandas dataframe.

        :param canonical_exposures: Canonical exposures
        :type canonical_exposures: pandas.DataFrame

        :param gul_items: GUL items
        :type gul_items: pandas.DataFrame
...
class OasisExposuresManager(implements(OasisExposuresManagerInterface)):
    ...
    def load_gul_items(self, canonical_exposures_profile, canonical_exposures_file_path, keys_file_path):
        ...
        try:
            ...
            for it in self.generate_gul_items(cep, canexp_df, keys_df):
                gulm_df = gulm_df.append([it])
        except OasisException as e:
            raise
        else:
            gulm_df['index'] = list(range(len(gulm_df)))
    ...
    def load_gul_master_data_frame(
        ...
        try:
            ...
            merged_df['index'] = pd.Series(data=list(range(len(merged_df.index))), dtype=object)
    ...
    def write_gul_files(self, oasis_model=None, **kwargs):
        ...
        gulm_df, canexp_df = self.load_gul_items(canonical_exposures_profile, canonical_exposures_file_path, keys_file_path)
