...
class OasisExposuresManager(implements(OasisExposuresManagerInterface)):
    ...
    def load_fm_master_data_frame(
        ...
        try:
            ...
            concurrent_tasks = (
                Task(get_fm_terms_by_level_as_list, args=(level_id, cgcp[level_id], canexp_df.copy(deep=True), canacc_df.copy(deep=True), pd.DataFrame(fm_df.where(fm_df['level_id'] == level_id).drop_duplicates().dropna(), dtype=object),), key=level_id)
                for level_id in fm_levels
            )
