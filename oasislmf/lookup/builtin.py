...
in the future we may want to improve on the management of files used to
generate the keys
tutorial for pandas and parquet
https://medium.com/towards-data-science/a-gentle-introduction-to-apache-arrow-with-apache-spark-and-pandas-bb19ffe0ddae
"""
import warnings
...
try:  # needed for rtree
    ...
    # > ...lib64/python3.8/site-packages/geopandas/_compat.py:112:
    #   UserWarning: The Shapely GEOS
    # > version (3.8.0-CAPI-1.13.1 ) is incompatible with the GEOS version
    #   PyGEOS was compiled with
    # > (3.10.3-CAPI-1.16.1). Conversions between both will be slow.
    # We're not in a position to fix these without compiling shapely and
    #   pygeos from source.
...
OPT_INSTALL_MESSAGE = ("install oasislmf with extra packages"
                       "by running 'pip install oasislmf[extra]'")
...
def nearest_neighbor(left_gdf, right_gdf, return_dist=False):
    """
    For each point in left_gdf, find closest point in right GeoDataFrame and
    return them.
...
def z_index(x, y):
    ...
    bits = int(max(np.floor(np.log2(x)) + 1, np.floor(np.log2(y)) + 1))
    index = 0
    for i in range(bits):
        index |= ((x >> i) & 1) << (2 * i)
        index |= ((y >> i) & 1) << (2 * i + 1)
...
class PerilCoveredDeterministicLookup(AbstractBasicKeyLookup):
    ...
    def process_locations(self, locations):
        ...
        if 'LocPerilsCovered' in locations.columns:
            ...
            peril_covered_column = 'PolPerilsCovered'

        split_df = (
            locations[peril_covered_column]
            .str.split(';')
            .apply(pd.Series)
            .stack()
        )
        ...
        keys_df = (
            locations.join(split_df)
            .merge(peril_groups_df)[['loc_id', 'peril_id']]
        )
        ...
        keys_df = (
            ...
            .merge(coverage_df, how="cross")
        )

        success_df = keys_df['peril_id'].isin(model_perils_covered)
        success_df_len = keys_df[success_df].shape[0]

        keys_df.loc[success_df, 'area_peril_id'] = np.arange(
            1, success_df_len + 1
        )

        keys_df.loc[success_df, 'vulnerability_id'] = np.arange(
            1, success_df_len + 1
        )
...
class Lookup(AbstractBasicKeyLookup, MultiprocLookupMixin):
    ...
    The aim of this class is to provide a data driven lookup capability that
    will be both flexible and efficient.

    it provide several generic function factory that can be define in the
    config under the "step_definition" key (ex:)
    "step_definition": {
        ...
        "vulnerability": {
            ...
            "parameters": {"file_path":
                            "%%KEYS_DATA_PATH%%/vulnerability_dict.csv",
                           "id_columns": ["vulnerability_id"]
                          }
        }
    }
    mapper key: is called the step_name,
        it will be added the the lookup object method once the function has
        been built
        it can take any value but make sure it doesn't collide with already
        existing method
    type: define the function factory to call.
        in the class for type <fct_type> the function factory called will be
        build_<fct_type>
    ...
    columns: are the column required to be able to apply the step.
        ...
        from the original Locations Dataframe that is not in any step will be
        drop to reduce memory consumption
    parameters: the parameter passed the the function factory.

    Once all the functions have been defined, the order in which they must be
    applied is defined in the config
    under the "strategy" key (ex:)
        "strategy": ["split_loc_perils_covered", "vulnerability"]

    It is totally possible to subclass Lookup in order to create your custom
    step or function factory
    for custom step:
    ...
    simply add it to your "strategy": ["split_loc_perils_covered",
                                        "vulnerability", "my_custom_step"]
    ...
    add your function definition to the "step_definition" with the required
    parameters
    "my_custom_step": {
            "type": "custom_type" ,
            "columns": [...],
    ...
    add your step to "strategy": ["split_loc_perils_covered", "vulnerability",
                                  "my_custom_step"]
    ...
    def set_step_function(
            ...
            step_name,
            step_config,
            function_being_set=None):
        """
        set the step as a function of the lookup object if it's not already
        done and return it.
        if the step is composed of several child steps, it will set the child
        steps recursively.

        Args:
            step_name (str): name of the strategy for this step
            step_config (dict): config of the strategy for this step
            function_being_set (set, None): set of all the strategy that are
            parent of this step
        ...
        if hasattr(self, step_name):
            ...
            if step_config['type'] == 'combine':
                # we need to build the child function
                if function_being_set is None:
                    # make sure we catch cyclic strategy definition
                    function_being_set = {step_name}
                elif step_name in function_being_set:
                    raise OasisException(
                        f"lookup config has a cyclic strategy definition "
                        f"{function_being_set} then {step_name} again"
                    )
                else:
                ...
                for child_step_name in step_config["parameters"]['strategy']:
                    child_fct = self.set_step_function(
                        ...
                        step_config=(
                            self.config['step_definition'][child_step_name]
                        ),
                        function_being_set=function_being_set
                    )
                    functions.append(
                        {'function': child_fct,
                         'columns': set(step_config.get("columns", []))}
                    )

                step_config['parameters']['strategy'] = functions

            step_function = getattr(
                self, f"build_{step_config['type']}"
    ...
    def process_locations(self, locations):
        # drop all unused columns and remove duplicate rows, find and rename
        # useful columns
        lower_case_column_map = {
            column.lower(): column for column in locations.columns}

        useful_cols = set(
            ['loc_id'] + sum(
                (step_config.get("columns", []) for step_config
                 in self.config['step_definition'].values()),
                []
            )
        )

        useful_cols_map = {
            lower_case_column_map[useful_col.lower()]: useful_col
        ...
        for step_name in self.config["strategy"]:
            ...
            if not needed_column.issubset(locations.columns):
                raise OasisException(
                    "Key Server Issue: missing columns "
                    f"{needed_column.difference(locations.columns)} for step "
                    f"{step_name}"
                )
        ...
        additional_columns = [
            'amplification_id', 'model_data', 'section_id',
            'intensity_adjustment', 'return_period'
        ]
    ...
    def to_abs_filepath(self, filepath):
        """
        replace placeholder r'%%(.+?)%%' (ex: %%KEYS_DATA_PATH%%) with the
        path set in self.config
        Args:
        ...
        for placeholder_key in placeholder_keys:
            filepath = filepath.replace(
                f'%%{placeholder_key}%%', self.config[placeholder_key.lower()]
            )
        return filepath
    ...
    def set_id_columns(df, id_columns):
        """
        in Dataframes, only float column can have nan values. So after a left
        join for example if you have nan values
        that will change the type of the original column into float.
        this function replace the nan value with the OASIS_UNKNOWN_ID and
        reset the column type to int
        """
    ...
    def build_interval_to_index(
        ...
        Args:
            ...
            side: define what index is returned (left or right) in case of
            equality with one of the interval boundary
        ...
        if isinstance(sorted_array, list):
            ...
            sorted_array = [
                float(val) for val in open(self.to_abs_filepath(sorted_array))
                if val.strip()
            ]

        else:
            raise OasisException(
                "sorted_array must be a list of the interval "
                "sorted or a path to a csv file containing those interval")

        if index_column_name is None:
            index_column_name = value_column_name + '_idx'

        def fct(locations):
            locations[index_column_name] = np.searchsorted(
                sorted_array, locations[value_column_name], side=side
            )

            empty_values = is_empty(locations, value_column_name)
            loc_empty = locations.loc[empty_values, value_column_name]
            locations.loc[empty_values, index_column_name] = loc_empty
            return locations
    ...
    def build_combine(id_columns, strategy):
        """
        build a function that will combine several strategy trying to achieve
        the same purpose by different mean into one.
        for example, finding the correct area_peril_id for a location with one
        method using (latitude, longitude)
        and one using postcode.
        each strategy will be applied sequentially on the location that steal
        have OASIS_UNKNOWN_ID in their id_columns after the precedent strategy

        Args:
            id_columns (list): columns that will be checked to determine if a
        ...
        def fct(locations):
            ...
            for child_strategy in strategy:
                if not child_strategy['columns'].issubset(locations.columns):
                    # needed column not present to run this strategy
                    continue
                locations = child_strategy['function'](locations)
                condition = locations[id_columns] != OASIS_UNKNOWN_ID
                is_valid = (condition).any(axis=1)
    ...
    def build_split_loc_perils_covered(model_perils_covered=None):
        """
        split the value of LocPerilsCovered into multiple line, taking peril
        group into account
        ...
        def fct(locations):
            for col in locations.columns:
                ...
                raise OasisException('missing PerilsCovered column'
                                     ' in location')

            split_df = (
                locations[perils_covered_column]
                .astype(str)
                .str.split(';')
                .apply(pd.Series)
                .stack()
            ...
            if model_perils_covered:
                ...
                location = location.merge(
                    df_model_perils_covered, left_on='peril_id',
                    right_on='model_perils_covered'
                )
    ...
    def build_prepare(**kwargs):
        ...
        support several simple DataFrame preparation:
            default: create the column if missing and replace the nan value
                ...
                if "default" in preparations:
                    if column_name not in locations.columns:
                        ...
                        fill_empty(
                            locations,
                            column_name,
                            preparations["default"]
                        )
                if 'max' in preparations:
                    locations.loc[
                        locations[column_name] > preparations['max'],
                        column_name
                    ] = preparations['max']

                if 'min' in preparations:
                    locations.loc[
                        locations[column_name] > preparations['min'],
                        column_name
                    ] = preparations['min']

                if 'type' in preparations:
                    locations[column_name] = (
                        locations[column_name].astype(preparations['type'])
                    )
    ...
    def build_rtree(
        ...
        !!!
        please note that this method is quite time consuming (specialy if you
        use the nearest point option
        if your peril_area are square you should use area_peril function
        fixed_size_geo_grid
        !!!

        file_path: is the path to the file containing the
        area_peril_dictionary.
            ...
            if you are new to geo data (in python) and want to learn more, you
            may have a look at this excellent course:
            https://automating-gis-processes.github.io/site/index.html

        file_type: can be any format readable by geopandas
        ('file', 'parquet', ...)
            see:
            https://geopandas.readthedocs.io/en/latest/docs/reference/io.html
            you may have to install additional library such as pyarrow for
            parquet

        id_columns: column to transform to an 'id_column'
        (type int32 with nan replace by -1)

        nearest_neighbor_min_distance: option to compute the nearest point if
        intersection method fails
            we use:
        ...
        if Point is None:
            raise OasisException(
                "shapely and geopandas modules are needed"" for rtree,"
        ...
        if hasattr(gpd, f"read_{file_type}"):
            ...
            gdf_area_peril = getattr(gpd, f"read_{file_type}")(
                self.to_abs_filepath(file_path), **area_peril_read_params
            )

        else:
            raise OasisException(
                f"Unregognised Geopandas read type {file_type}"
            )
        ...
        def get_area(locations, gdf_area_peril):
            # this conversion could be done in a separate step allowing more
            # posibilities for the geometry
            null_gdf = (
                locations["longitude"].isna() | locations["latitude"].isna()
            )
            ...
            if nearest_neighbor_min_distance > 0:
                ...
                if gdf_loc_na.shape[0]:
                    ...
                    nearest_neighbor_df = nearest_neighbor(
                        gdf_loc_na, gdf_area_peril, return_dist=True
                    )

                    gdf_area_peril.set_geometry(
                        base_geometry_name, inplace=True
                    )

                    valid_nearest_neighbor = (
                        nearest_neighbor_df['distance']
                        <= nearest_neighbor_min_distance
                    )

                    common_col = list(
                        set(gdf_loc_na.columns)
                        & set(nearest_neighbor_df.columns)
                    )
                    gdf_loc.loc[valid_nearest_neighbor.index, common_col] = (
                        nearest_neighbor_df.loc[
                            valid_nearest_neighbor, common_col
                        ]
                    )
            ...
            gdf_loc = gdf_loc.drop(
                columns=['index_right', 'index_left'], errors='ignore'
            )
        ...
        def fct(locations):
            if 'peril_id' in gdf_area_peril.columns:
                ...
                res = [
                    locations[~locations["peril_id"].isin(peril_id_covered)]
                ]

                for peril_id in peril_id_covered:
                    res.append(
                        get_area(
                            locations.loc[locations["peril_id"] == peril_id],
                            gdf_area_peril.loc[
                                gdf_area_peril["peril_id"] == peril_id
                            ]
                            .drop(columns=["peril_id"])
                        )
                    )
    ...
    def build_fixed_size_geo_grid_multi_peril(perils_dict):
        ...
        ----------
        perils_dict: dict
                     Dictionary with `peril_id` as key and
                     `fixed_size_geo_grid` parameter dict as
                     value. i.e
                     `{'peril_id' : {fixed_size_geo_grid parameters}}`
        """
        def fct(locs_peril):
            start_index = 0
            ...
            for peril_id, fixed_geo_grid_params in perils_dict.items():
                curr_grid_fct = Lookup.build_fixed_size_geo_grid(
                    **fixed_geo_grid_params
                )

                curr_locs_peril = locs_peril[
                    locs_peril['peril_id'] == peril_id
                ]
    ...
    def build_fixed_size_z_index_geo_grid_multi_peril(perils_dict):
        ...
        def fct(locs_peril):
            ...
            for index, (peril_id, fixed_geo_grid_params) in enumerate(
                perils_dict.items()
            ):
                curr_grid_fct = Lookup.build_fixed_size_z_index_geo_grid(
                    **fixed_geo_grid_params
                )
    ...
    def build_merge(self, file_path, id_columns=[], **kwargs):
        """
        this method will merge the locations Dataframe with the Dataframe
        present in file_path
        All non match column present in id_columns will be set to -1

        this is an efficient way to map a combination of column that have a
        finite scope to an idea.
        """

        df_to_merge = pd.read_csv(self.to_abs_filepath(file_path), **kwargs)
        df_to_merge.rename(
            columns={column: column.lower() for column in df_to_merge.columns},
            inplace=True
        )
        ...
        def merge(locations: pd.DataFrame):
            rename_map = {
                col.lower(): col
                for col in locations.columns
                if col.lower() in df_to_merge.columns
            }

            locations = locations.merge(
                df_to_merge.rename(columns=rename_map), how='left'
            )
