...
def get_default_source_oed_loc_profile(data_fp=static_data_fp):
    with io.open(os.path.join(static_data_fp, 'oed-loc-profile.json'), 'r', encoding='utf-8') as f:
        return json.load(f)


def get_default_source_oed_acc_profile(data_fp=static_data_fp):
    with io.open(os.path.join(static_data_fp, 'oed-acc-profile.json'), 'r', encoding='utf-8') as f:
        return json.load(f)
