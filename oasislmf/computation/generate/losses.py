    ...
    * Stores the analysis_settings.json in the output directory
    ...
    step_params = [
        ...
        {'name': 'analysis_settings_json', 'flag': '-a', 'is_path': True, 'pre_exist': True,
            'required': True, 'help': 'Analysis settings JSON file path'},
        {'name': 'model_settings_json', 'flag': '-M', 'is_path': True, 'pre_exist': False,
            'required': False, 'help': 'Model settings JSON file path'},
        ...
        {'name': 'model_package_dir', 'flag': '-p', 'is_path': True,
            'pre_exist': False, 'help': 'Path containing model specific package'},
    ...
    def run(self):
        ...
        if not ri:
            ...
            for fp in [os.path.join(model_run_fp, fn)
                       for fn in contents if re.match(r'RI_\d+$', fn) or re.match(r'input$', fn)]:
        ...
        if not analysis_settings.get('number_of_samples'):
            ...
            self.logger.info(
                f"Loaded samples from model_settings file: 'model_default_samples = {default_model_samples}'")
...
class GenerateLossesPartial(GenerateLossesDir):
    ...
    step_params = GenerateLossesDir.step_params + [
        ...
        {'name': 'ktools_alloc_rule_gul', 'default': KTOOLS_ALLOC_GUL_DEFAULT,
            'type': int, 'help': 'Set the allocation used in gulcalc'},
        {'name': 'ktools_alloc_rule_il', 'default': KTOOLS_ALLOC_IL_DEFAULT, 'type': int,
            'help': 'Set the fmcalc allocation rule used in direct insured loss'},
        {'name': 'ktools_alloc_rule_ri', 'default': KTOOLS_ALLOC_RI_DEFAULT,
            'type': int, 'help': 'Set the fmcalc allocation rule used in reinsurance'},
        ...
        {'name': 'modelpy', 'default': True, 'type': str2bool, 'const': True,
            'nargs': '?', 'help': 'use getmodel python version instead of c++ version'},
        {'name': 'gulpy', 'default': True, 'type': str2bool, 'const': True,
            'nargs': '?', 'help': 'use gulcalc python version instead of c++ version'},
        ...
        {'name': 'fmpy_sort_output', 'default': False, 'type': str2bool,
        ...
        {'name': 'process_number', 'default': None, 'type': int,
            'help': 'Partition number to run, if not set then run all in a single script'},
        {'name': 'max_process_id', 'default': -1, 'type': int,
            'help': 'Max number of loss chunks, defaults to `ktools_num_processes` if not set'},
        {'name': 'ktools_fifo_queue_dir', 'default': None, 'is_path': True,
            'help': 'Override the path used for fifo processing'},
    ]
        ...
        with setcwd(model_run_fp):
            try:
                if self.process_number:
                    ...
                    self.logger.info(
                        'All {} Loss chunks generated in {}'.format(
                            bash_params['max_process_id'], model_run_fp))
...
class GenerateLossesDeterministic(ComputationStep):
    ...
    def run(self):
        ...
        il = all(p in os.listdir(self.oasis_files_dir)
                 for p in ['fm_policytc.csv', 'fm_profile.csv', 'fm_programme.csv', 'fm_xref.csv'])
        ...
        if self.fmpy:
            with setcwd(self.oasis_files_dir):
                check_call(
                    f"{get_fmcmd(self.fmpy)} -a {self.ktools_alloc_rule_il} --create-financial-structure-files -p {output_dir}",
                    shell=True)
...
class GenerateLossesDummyModel(GenerateDummyOasisFiles):
    ...
    step_params = [
        {'name': 'analysis_settings_json', 'flag': '-z', 'is_path': True, 'pre_exist': True,
            'required': True, 'help': 'Analysis settings JSON file path'},
        {'name': 'ktools_num_processes', 'flag': '-n', 'type': int, 'default': KTOOLS_NUM_PROCESSES,
    ...
    def _validate_analysis_settings(self):
        ...
        if self.analysis_settings.get('ri_output'):
            warnings.warn(
                'Generating reinsurance losses with dummy model not currently supported. Ignoring ri_output in analysis settings JSON.')
        ...
        for idx, param in enumerate(loss_params):
            if self.analysis_settings.get(param['output']):
                ...
                self.analysis_settings[param['summary']][:] = [
                    x for x in self.analysis_settings[param['summary']] if not x.get('oed_fields')]
                ...
                if self.analysis_settings[param['output']]:
                    ...
                    if self.analysis_settings[param['summary']][0]['leccalc']['return_period_file']:
                        warnings.warn(
                            f'Return period file is not generated. Please use "return_periods" field in analysis settings JSON.')
        ...
        if self.analysis_settings.get('model_settings').get('use_random_number_file'):
            if self.num_randoms == 0:
                warnings.warn(
                    'Ignoring use random number file option in analysis settings JSON as no random number file will be generated.')
