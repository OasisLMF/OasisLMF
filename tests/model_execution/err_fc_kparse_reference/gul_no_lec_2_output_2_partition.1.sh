...
tee < fifo/gul_S2_summary_P2 > /dev/null & pid2=$!
( summarypy -m -t gul  -1 fifo/gul_S1_summary_P2 -2 fifo/gul_S2_summary_P2 < fifo/gul_P2 ) 2>> $LOG_DIR/stderror.err  &

( ( evepy 2 2 | gulmc --socket-server='False' --random-generator=1  --model-df-engine='oasis_data_manager.df_reader.reader.OasisPandasReader' --vuln-cache-size 200 -S0 -L0 -a1  > fifo/gul_P2  ) 2>> $LOG_DIR/stderror.err ) &  pid3=$!
