...
tee < /tmp/%FIFO_DIR%/fifo/il_S1_summary_P1 /tmp/%FIFO_DIR%/fifo/il_S1_elt_ord_P1 /tmp/%FIFO_DIR%/fifo/il_S1_selt_ord_P1 > /dev/null & pid4=$!
tee < /tmp/%FIFO_DIR%/fifo/il_S2_summary_P1 /tmp/%FIFO_DIR%/fifo/il_S2_plt_ord_P1 > /dev/null & pid5=$!
tee < /tmp/%FIFO_DIR%/fifo/il_S3_summary_P1 work/il_S3_summary_palt/P1.bin work/il_S3_summary_altmeanonly/P1.bin > /dev/null & pid6=$!
tee < /tmp/%FIFO_DIR%/fifo/il_S3_summary_P1.idx work/il_S3_summary_palt/P1.idx > /dev/null & pid7=$!
...
( ( evepy 1 8 | gulmc --socket-server='False' --random-generator=1  --model-df-engine='oasis_data_manager.df_reader.reader.OasisPandasReader' --vuln-cache-size 200 -S100 -L100 -a1  | fmpy -a2 > /tmp/%FIFO_DIR%/fifo/il_P1  ) 2>> $LOG_DIR/stderror.err ) & pid8=$!
