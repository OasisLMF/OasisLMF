...
tee < /tmp/%FIFO_DIR%/fifo/il_S2_summary_P4 work/il_S2_summary_palt/P4.bin work/il_S2_summaryleccalc/P4.bin > /dev/null & pid3=$!
tee < /tmp/%FIFO_DIR%/fifo/il_S2_summary_P4.idx work/il_S2_summary_palt/P4.idx work/il_S2_summaryleccalc/P4.idx > /dev/null & pid4=$!
...
tee < /tmp/%FIFO_DIR%/fifo/gul_S1_summary_P4 work/gul_S1_summary_palt/P4.bin work/gul_S1_summaryleccalc/P4.bin > /dev/null & pid5=$!
tee < /tmp/%FIFO_DIR%/fifo/gul_S1_summary_P4.idx work/gul_S1_summary_palt/P4.idx work/gul_S1_summaryleccalc/P4.idx > /dev/null & pid6=$!
tee < /tmp/%FIFO_DIR%/fifo/gul_S2_summary_P4 work/gul_S2_summary_palt/P4.bin work/gul_S2_summaryleccalc/P4.bin > /dev/null & pid7=$!
tee < /tmp/%FIFO_DIR%/fifo/gul_S2_summary_P4.idx work/gul_S2_summary_palt/P4.idx work/gul_S2_summaryleccalc/P4.idx > /dev/null & pid8=$!
...
( ( evepy 4 8 | gulmc --random-generator=1  --model-df-engine='oasis_data_manager.df_reader.reader.OasisPandasReader' --vuln-cache-size 200 -S0 -L0 -a1  | tee /tmp/%FIFO_DIR%/fifo/gul_P4 | fmpy -a2 > /tmp/%FIFO_DIR%/fifo/il_P4  ) 2>> $LOG_DIR/stderror.err ) & pid9=$!
