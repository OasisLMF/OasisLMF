...
tee < fifo/gul_S1_summary_P4 > /dev/null & pid1=$!
( summarypy -m -t gul  -1 fifo/gul_S1_summary_P4 < fifo/gul_P4 ) 2>> $LOG_DIR/stderror.err  &

( ( evepy 4 20 | gulmc --socket-server='False' --random-generator=1  --model-df-engine='oasis_data_manager.df_reader.reader.OasisPandasReader' --vuln-cache-size 200 -S100 -L100 -a1  > fifo/gul_P4  ) 2>> $LOG_DIR/stderror.err ) &  pid2=$!
