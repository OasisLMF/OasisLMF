...
def _make_compute_event_losses_args(event_rp, item_rp, item_intensity_adjustment=0,
    ...
    if sample_size > 0:
        vuln_rndms_base[0, :] = np.linspace(0.05, 0.95, sample_size)
        haz_rndms_base[0, :] = np.linspace(0.05, 0.95, sample_size)
    vuln_adj = np.ones(1, dtype=oasis_float)      # no per-vulnerability adjustment
