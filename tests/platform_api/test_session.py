...
class APISessionTests(unittest.TestCase):
    ...
    def test_upload_file_different_serializer_file_name(self):
        fake_response = Mock()
        fake_response.raise_for_status.return_value = None

        with (
            patch("oasislmf.platform_api.session.MultipartEncoder") as mock_encoder,
            ...
            self.session.upload(
                ...
                filepath=str(Path(__file__).resolve().parent / "data" / "currency_conversion_list.json"),
                content_type="application/json",
                serializer_field_name="json_file",
            )

            mock_encoder.assert_called_once()
            fields = mock_encoder.call_args.kwargs["fields"]

            assert "json_file" in fields
            assert "file" not in fields

            _, post_kwargs = mock_post.call_args
            assert "serializer_field_name" not in post_kwargs

    def test_upload_file_standard_serializer_file_name(self):
        fake_response = Mock()
        fake_response.raise_for_status.return_value = None
